...
| New Rust/Tauri lite exe after release-size tuning, monitoring events, window enumeration, window capture, window-source scan wiring, missing-source and monitoring-error status messaging, remembered-window resolution status messaging, source preview cache, basic multi-source preview panel, source-preview failure status messaging, DWM preview overlay handoff and lifecycle verification, evidence labels, OCR text-row matching, OCR Cargo feature boundary, large-frame template coarse/refine, template worker scheduling, alarm beep runtime, startup shortcut backend, single-instance wake protocol, tray lifecycle backend, packaged start-minimized smoke, profile template PNG/JPG/JPEG/BMP/WEBP importing with PNG normalization and clipboard image/path paste, screenshot-as-template capture, profile target reorder/remove/clear backend commands, profile target enable/toggle-all helpers, profile-to-watch-config construction, profile scan/session commands, profile hit-count auto-update, profile read snapshots, profile source/match persistence, Python-compatible `state.json` window geometry restore/save, a basic profile frontend panel, OCR backend interface wiring, explicit OCR backend probe plumbing, legacy app_data migration, frontend behavior tests including Python-compatible window geometry/layout math, custom check indicator scaling, same-card repeat-click opening, autohide target-gallery scrollbar state, profile source option shape, monitoring event transition, missing-source scan/monitoring status text, remembered-window resolution status text, source-preview failure status text, source-preview refresh gating, DWM bitmap-failure fallback, monitoring error text, restore-overlay source-preview covering, target hit-count context-menu state, target row action state, drag/drop target insert-index behavior, real DWM thumbnail smoke, template parity benchmarking, exact-flat-template integral scoring, shared gray/scale frame caching, and native OCR backend feature wiring kept out of lite | `target\release\screen-watch-ocr-tauri.exe` after `npx tauri build --no-bundle --ci` | 3,567,616 bytes, about 3.40 MiB |
| New Rust/Tauri full exe with native `pure-onnx-ocr` backend linked through the lazy OCR worker and OCR models external | `target\release\screen-watch-ocr-tauri.exe` after `npm run package:portable:full` | 8,602,624 bytes, about 8.20 MiB |
