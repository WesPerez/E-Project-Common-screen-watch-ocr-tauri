...
| Screen preview captures a real frame | `test_screen_preview_captures_real_frame` | Windows GDI region capture command done; the current desktop capture gate and packaged WebView source-preview smoke pass with real screen/window preview frames |
...
| App-window list uses stable title sorting, duplicate ordinals, legacy keys, display labels, and does not list the old/new monitor app itself | `list_app_windows`, `window_key`, selection tests | Done in Tauri `window_sources`; desktop enumeration gate passed. A Tauri backend test locks the self-filter for empty titles, `ScreenWatchOCR`, `Screen Watch OCR`, `Screen Watch OCR Tauri`, and `Program Manager`, while the migration verifier also requires the current-process PID filter to remain in the Win32 enumeration path |
...
| Selected window-only monitoring works without selected monitors | `test_detector_config_allows_window_only_source` | Core source planner, one-shot window scan, persistent window monitoring, profile window-scan desktop gates, and packaged WebView one-shot/monitoring workflows pass with only an app-window source selected |
...
| Open evidence directory creates the Python-compatible screenshot directory before launching the system opener | Python `open_evidence` | Done in Tauri `open_evidence_dir_at`: backend tests verify it creates and returns `ScreenWatchOCR\screenshots`, does not switch to the legacy `alerts` directory, and surfaces shell-open failures to the frontend command result. The frontend now routes the visible `打开证据目录` button through tested `evidenceDirectoryStatusText` / `evidenceDirectoryLogText` helpers so the opened screenshots path remains visible in status/log text; `frontendEvidenceDirectoryContract` locks the button, invoke, helper tests, and backend command path together |
| Beep duration does not restart while already beeping | Python `start_beep` | Done in `screen-watch-core::audio::BeepThrottle` and Tauri `AlarmBeepState`; one-shot scans and monitoring sessions trigger it after alert hits; migration verifier `audioAlarmParityContract` locks the Python and Tauri trigger paths |
| Beep volume clamps and waveform amplitude changes | `test_beep_volume_is_clamped_and_changes_wave_amplitude` | Done in `screen-watch-core::audio` with dependency-free PCM WAV generation and amplitude tests; the verifier also locks Python `winsound.PlaySound(..., SND_MEMORY)` against Tauri `PlaySoundW`/`SND_MEMORY` |
...
| DWM sync loop schedules only once | `test_dwm_sync_loop_schedules_once` | Basic Tauri behavior done through the existing single frontend preview timer; DWM rects are refreshed on that path, and the current real DWM desktop gate verifies thumbnail registration, update reuse, and clear/unregister behavior |
| DWM preview falls back when widget is not visible | `test_dwm_preview_falls_back_when_widget_not_visible_yet` | Frontend computes visible card rects through the tested `visiblePreviewRect` helper, clips partially offscreen WebView frames, and skips DWM sync when the frame or viewport is hidden/tiny; bitmap previews remain the fallback. Frontend tests also verify that a card already handed off to DWM stays healthy if the bitmap capture path fails, and packaged WebView source-preview smoke covers visible scroll/resize/restore behavior |
| DWM preview unregisters before hide/minimize/layout drag | DWM suspend/hide tests | DWM overlays are cleared on preview disable, hidden page, scroll/resize, source-card rerender, stale-source retention, and Tauri close/destroy events. Fake-backend tests verify stale-key retention, source-handle replacement, and clear-time unregister behavior; the current real DWM desktop gate verifies native clear/unregister on this Windows desktop |
...
| Startup shortcut uses `--start-minimized` | `test_startup_arguments_start_minimized` | Done in Tauri `startup` module; Windows Startup `.lnk` path uses the distinct `屏幕监控OCR Tauri.lnk` name, packaged app arguments use `--start-minimized`, and explicit status/toggle commands are exposed. The Windows-only `startup_manager_writes_reads_and_removes_isolated_shortcut` test now also creates a real shortcut under a temporary Startup-shaped directory, reads it back through `WScript.Shell`, verifies target/arguments/working directory, disables it, and removes the temp link without mutating the user's actual Startup folder |
...
| Single exe runtime boundary is explicit | final executable smoke plus local runtime audit | Done: the final single exe is proven on this WebView2-present Windows machine, with Microsoft Edge WebView2 Runtime `149.0.4022.98` found under `C:\Program Files (x86)\Microsoft\EdgeWebView\Application`; the audit documents that machines without WebView2 need the installer or WebView2 installed first |
