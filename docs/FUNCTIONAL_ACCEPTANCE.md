...
| Open evidence directory creates the Python-compatible screenshot directory before launching the system opener | Python `open_evidence` | Done in Tauri `open_evidence_dir_at`: backend tests verify it creates and returns `ScreenWatchOCR\screenshots`, does not switch to the legacy `alerts` directory, and surfaces shell-open failures to the frontend command result |
