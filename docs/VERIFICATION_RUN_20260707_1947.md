...
| Python baseline: `PYTHONPATH=src; python -m unittest discover -s tests -t . -v` and `python -m screen_watch app --smoke-test` | Passed. The Python baseline ran 98 tests with final `OK`; `app --smoke-test` returned `{"ok": true, "monitors": 3}`. Python/Tk emitted intermittent destroyed-window callback noise during teardown, matching prior baseline behavior. |
| `npm run python:profile:compat` | Passed. Result: `docs\manual-gate-evidence\logs\python-profile-compat-smoke-20260707-200613-result.json`. Old Python loaded the Tauri-shaped shared profile/state/template fixture and saved required Python-compatible keys; the smoke again proved old Python stale saves can overwrite Tauri-style post-load updates and future unknown fields, so simultaneous old/new writes remain unsupported. |
| `npm run template:parity` | Passed. Python/OpenCV flat benchmark: 54ms, 8/8. Python/OpenCV textured benchmark: 48ms with the known 4/8 baseline miss. Rust release flat benchmark: 78ms, 8/8. Rust release textured benchmark: 473ms, 8/8. |
| `npm run production:template:smoke -- -SkipParity` | Passed. Read the real shared `C:\Users\Wes\AppData\Local\ScreenWatchOCR\profiles\profile_1.json` and templates without modifying them; 18 enabled template targets matched 18/18 on a synthetic 2560x1440 placement in 6621ms with threshold 0.90, scales 1.0, and template_workers 2. |
| `powershell -ExecutionPolicy Bypass -File scripts\verify-migration.ps1 -SkipPython -SkipFrontend -SkipRelease -IncludeDesktopSmoke` | Passed. Rust core `124 passed, 3 ignored`; Tauri `92 passed, 16 ignored`; OCR feature `28 passed`; desktopSmoke `16 gates`; final single-file contract remained `3587584 bytes`, SHA-256 `8986F1168578CF6B564229E3D80C12DC1E8809138B0786B38C8DD99B46E3BF9A`, WindowsGui. Covered real screen capture, monitor listing, screen/window one-shot scans, profile screen/window workflows, screenshot-as-template capture, persistent screen/window monitoring, app-window enumeration/preview/frame capture, and real DWM thumbnail register/update/clear. |
| `npm run ocr:text:parity` | Passed. Result: `docs\manual-gate-evidence\logs\ocr-text-parity-smoke-20260707-201541-result.json`. Compared old Python `Detector._ocr` semantics for case-insensitive contains, min_score miss, case-sensitive miss/hit, box flattening, missing box behavior, and Unicode contains against Rust OCR text detection plus ScanEngine OCR backend tests. |
| `npm run ocr:corpus:smoke` | Passed. Result: `docs\manual-gate-evidence\logs\ocr-corpus-smoke-20260707-201558-result.json`. Used retained external PP-OCRv5-style English and Chinese ONNX assets under `target\ocr-model-smoke`, generated local PNGs under `target\ocr-corpus-smoke`, and recognized READY, ALERT 42, OCR TEST, SCAN COMPLETE, ERROR 100%, 准备好了, 开始监控, 屏幕监控, and 发现异常. |
