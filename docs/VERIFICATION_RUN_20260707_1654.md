...
| `powershell -ExecutionPolicy Bypass -File scripts\verify-migration.ps1 -SkipPython -SkipFrontend -SkipRelease` after template file boundary contract hardening | Passed. Summary includes Rust core `124 passed, 3 ignored`, Tauri `92 passed, 16 ignored`, OCR feature `28 passed`, `legacyTemplateFileBoundaryContract: passed`, and final `singleFileDeliverableContract: 3587584 bytes, 200C0C8E..., WindowsGui`. |
| `powershell -ExecutionPolicy Bypass -File scripts\verify-migration.ps1 -SkipPython -SkipFrontend -SkipRelease` after manual evidence risk wording hardening | Passed. Summary includes `manualGateEvidenceRiskContract: passed`, which prevents legacy-profile evidence from claiming the already-passed late-start remembered-window gate is uncovered and prevents production-template evidence from implying separately passed WebView/tray/installer/OCR gates are uncovered. |
| `powershell -ExecutionPolicy Bypass -File scripts\verify-migration.ps1 -SkipPython -SkipFrontend -SkipRelease` after acceptance status hardening | Passed. Summary includes `acceptanceCurrentStatusContract: passed`, which keeps `docs\ACCEPTANCE.md` anchored to the current 19/19 manual-gate status, final exe identity, and real remaining validation boundaries instead of stale future-gate wording. |
| `npm run test:frontend` after evidence-directory frontend hardening | Passed. Frontend unit tests increased to `105 passed`, including opened screenshots path status/log text and safe missing-path fallback coverage for the `打开证据目录` workflow. |
| `powershell -ExecutionPolicy Bypass -File scripts\verify-migration.ps1 -SkipPython -SkipRelease` after evidence-directory frontend hardening | Passed. Summary includes frontend `105 passed`, `frontendEvidenceDirectoryContract: passed`, and the same final `singleFileDeliverableContract: 3587584 bytes, 200C0C8E..., WindowsGui`. |
...
| Template file boundary | Proven by `legacyTemplateFileBoundaryContract`: Python import/delete/clear behavior, Rust profile helpers, Tauri backend gallery workflow, and acceptance docs all require generated/imported template files to live under `templates/` and require remove/clear operations to preserve external target paths. |
| Manual evidence drift | Current evidence records and the WebView evidence generator now say the legacy-profile-at-startup gate is complemented by the separate passed legacy-late-window gate, and the production-template evidence now says WebView/tray/installer/OCR are covered by separate passed records instead of implying they remain missing. |
| Acceptance checklist drift | `docs\ACCEPTANCE.md` now carries a current status snapshot and a remaining-validation-boundaries section; the verifier rejects stale future-gate wording for already-covered monitoring lifecycle, alert evidence, performance comparison, OCR-blocked, or tray-missing states. |
| Evidence directory frontend path | Proven by frontend tests and `frontendEvidenceDirectoryContract`: the visible `打开证据目录` button still invokes `open_evidence_dir`, shows the returned `ScreenWatchOCR\screenshots` path in status text, appends the same path to the event log, and keeps a safe fallback if a future backend returns no path. |
