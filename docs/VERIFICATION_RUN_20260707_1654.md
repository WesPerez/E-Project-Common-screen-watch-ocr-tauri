...
| Monitoring freeze/restart concern | Proven for start/stop/restart by packaged WebView smoke and for a 600000ms monitoring soak. Latest recorded soak: tick delta `2149`, hit delta `537`, progress-log delta `46`, stopped with the button restored to start. |
...
| UI compactness/resizable panes | Proven by frontend layout tests and WebView splitter drag smoke. Very narrow/mobile-like widths are not exhaustively visually sampled. |
