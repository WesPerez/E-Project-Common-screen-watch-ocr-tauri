...
| `powershell -ExecutionPolicy Bypass -File scripts\verify-migration.ps1 -SkipPython -SkipFrontend -SkipRelease` after manual evidence risk wording hardening | Passed. Summary includes `manualGateEvidenceRiskContract: passed`, which prevents legacy-profile evidence from claiming the already-passed late-start remembered-window gate is still missing. |
...
| Manual evidence drift | Current evidence records and the WebView evidence generator now say the legacy-profile-at-startup gate is complemented by the separate passed legacy-late-window gate, instead of claiming that late-start coverage is still missing. |
