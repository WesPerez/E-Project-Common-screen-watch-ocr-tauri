...
| Production template smoke | Latest continuation rerun against the real shared `profile_1.json`: the Rust-only production smoke matched 18/18 on 2560x1440 synthetic placement in 7319ms with threshold 0.90, scales 1.0, and template_workers 2; the new same-frame Python-vs-Rust production parity smoke `production-template-parity-smoke-20260707-220324-result.json` had old Python Detector match 18/18 in 258ms and Rust match the same 18/18 ids in 8422ms with no Rust-missing Python hits |
