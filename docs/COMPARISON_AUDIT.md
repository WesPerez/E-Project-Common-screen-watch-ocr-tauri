...
Last updated: 2026-07-07 19:03 +08:00
...
| --- | --- |
| Latest continuation audit | Fresh current-exe continuation run is recorded in `docs\VERIFICATION_RUN_20260707_1903.md`. It passed current-hash WebView visual smoke, late-start remembered-window smoke, 600-second monitoring soak, packaged smoke rerun, and Tauri-owned tray menu smoke. A current-hash Python/Tauri coexistence rerun was safely blocked before launching because the old Python default port was already owned by an existing app |
...
| Main migration verifier | Current focused verifier contract locks `release-single\ScreenWatchOCRTauri.exe` at 3,587,584 bytes with SHA-256 `98F47746...` and WindowsGui subsystem. Earlier full verifier evidence passed Python 98, Rust core 121 / 3 ignored, Tauri 88 / 16 ignored, OCR feature 25 / 2 ignored, frontend 103, frontend production build, lite release build, dependency boundaries, `requiredRealGates: 19 workspace gates, 2 OCR gates`, and `liteSizeGate: passed`. Later focused contracts raised frontend coverage to 105 tests and locked visible workflow, UI surface, default settings, failure guards, template file boundaries, profile persistence, audio parity, app-window self-filtering, and evidence-directory behavior. The delivered `release-single\ScreenWatchOCRTauri.exe` is the separately WebView/packaged/tray-verified single-file artifact; release builds are not treated as bit-for-bit reproducible evidence |
...
| Packaged smoke | Current rerun against final SHA-256 `98F47746...` verified PE subsystem WindowsGui (2), start-minimized, legacy app_data migration, legacy geometry restore, close-to-tray, and second-instance wake with isolated appdata/ports using `release-single\ScreenWatchOCRTauri.exe`; one same-hash automation attempt timed out waiting for the posted `WM_CLOSE` to hide the main window, then the immediate unchanged rerun passed |
| Tray menu smoke | Current rerun against final SHA-256 `98F47746...` passed Tauri-owned native menu `Show Tauri` and `Exit Tauri`; tray menu PID matched Tauri PID `23784`, exit code was 0, and old Python tray/processes were not touched |
| Python/Tauri coexistence smoke | Previous full coexistence proof launched old Python SHA-256 `A5689E...` and earlier Tauri SHA-256 `200C0C8E...` together against one shared isolated `ScreenWatchOCR` data root in `coexistence-smoke-20260707-161624-result.json`. Process trees were distinct, both visible main windows were detected, default ports `47627` and `47628` were simultaneously busy, cross-protocol commands were rejected both ways, each app acknowledged only its own command, each second instance exited 0, and Tauri WebView2 children used a smoke-owned user data folder. Current SHA-256 `98F47746...` coexistence rerun stopped before launching because the old Python default port `47627` was already busy, recorded in `coexistence-smoke-20260707-182936-result.json`; no old Python process was killed |
...
| WebView visual smoke | Current full rerun `webview-visual-smoke-20260707-183233-result.json` explicitly launched final SHA-256 `98F47746...` through `--exe-path .\release-single\ScreenWatchOCRTauri.exe` and passed legacy Python profile restore/scan/monitoring, source preview, template gallery import/edit/reorder/delete/clear/capture with generated PNG/JPG/JPEG/BMP/WebP path imports, clipboard bitmap and file-list paste, one-shot scan with evidence output, OCR-lite raw config rejection, monitoring start/stop/restart with progress logs, and all resizable layout splitter drags. Separate current rerun `webview-visual-smoke-20260707-183408-result.json` also passed the late-start remembered-window workflow after Tauri had already loaded the legacy profile |
| WebView monitoring soak | Current final SHA-256 `98F47746...` ran profile monitoring for 600,000ms in `webview-visual-smoke-20260707-184743-result.json` with 300 UI samples, tick delta 1109, hit delta 1109, progress-log delta 47, distinct tick counts across the samples, and stopped with the button restored to `开始监控` |
...
| Tick/event logs while monitoring | Proven | frontend tests, `monitor-session` contract, current packaged WebView restart smoke progress rows, current packaged 600s soak with tick delta 1109 and progress-log delta 47 | Log cadence depends on capture speed and configured interval |
...
| Close hides to tray | Proven with residual automation flake | current packaged smoke rerun passed close-to-tray and second-instance wake; a same-hash automation attempt immediately before it timed out waiting for the posted `WM_CLOSE` to hide the main window | Watch for recurrence in repeated packaged-smoke loops or manual close-to-tray trials |
