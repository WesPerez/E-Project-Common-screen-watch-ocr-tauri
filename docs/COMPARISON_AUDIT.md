...
Last updated: 2026-07-07 15:38 +08:00
...
| WebView monitoring soak | Previous final SHA-256 `6363339...` ran profile monitoring for 600,000ms with 300 UI samples, tick delta 1117, hit delta 1117, progress-log delta 47, and stopped with the button restored to `开始监控`; after the OCR-lite guard, the current final SHA-256 `B7356D3...` reran the shorter packaged monitoring restart smoke successfully |
...
| Tick/event logs while monitoring | Proven | frontend tests, `monitor-session` contract, current packaged WebView restart smoke progress rows, current packaged 600s soak with tick delta 1117 and progress-log delta 47 | Log cadence depends on capture speed and configured interval |
