...
Last updated: 2026-07-07 23:03 +08:00
...
| WebView monitoring soak | Current final SHA-256 `1D03E007...` ran the visible profile monitoring workflow for 60,000ms in `webview-visual-smoke-20260707-225749-result.json` with 31 UI samples, tick delta 109, hit delta 109, progress-log delta 46, and stopped with the button restored to `开始监控`. Previous final SHA-256 `8986F116...` ran the same gate for 600,000ms in `webview-visual-smoke-20260707-193601-result.json` with 300 UI samples, tick delta 2106, hit delta 526, progress-log delta 46, and distinct tick counts across the samples. The 600-second soak was intentionally not rerun after the final exact-gray fast-path rebuild because the user allowed skipping that longer wait |
...
| Persistent monitoring start/stop/status | Proven | monitor session tests, desktop monitoring gates, current packaged WebView monitoring restart smoke, current packaged 60s monitoring soak, historical 600s packaged soak before the final exact-gray rebuild | Multi-hour production soak and broad third-party window matrix are not recorded |
...
| Tick/event logs while monitoring | Proven | frontend tests, `monitor-session` contract, current packaged WebView restart smoke progress rows, current packaged 60s soak with tick delta 109 and progress-log delta 46, plus historical 600s packaged soak with tick delta 2106 and progress-log delta 46 | Log cadence depends on capture speed and configured interval |
