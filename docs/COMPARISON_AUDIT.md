...
| Python/Tauri coexistence smoke | Current full coexistence proof launched old Python SHA-256 `A5689E...` and current final Tauri SHA-256 `8986F116...` together against one shared isolated `ScreenWatchOCR` data root in `coexistence-smoke-20260707-213630-result.json`. Process trees were distinct, both visible main windows were detected, default ports `47627` and `47628` were simultaneously busy, cross-protocol commands were rejected both ways, each app acknowledged only its own command, each second instance exited 0, and Tauri WebView2 children used a smoke-owned user data folder. The smoke stopped only its own launched process trees and removed only its own temp root |
