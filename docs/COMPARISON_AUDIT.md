...
Last updated: 2026-07-07 12:43 +08:00
...
| Python/Tauri coexistence smoke | previous final SHA-256 `426BE3C...` and old Python SHA-256 `A5689E...` were launched together against one shared isolated `ScreenWatchOCR` data root before the target select-all label rebuild. Process trees were distinct (`ScreenWatchOCR` PyInstaller onefile tree vs `ScreenWatchOCRTauri` plus WebView2 tree), both visible main windows were detected, default ports `47627` and `47628` were simultaneously busy, cross-protocol commands were rejected both ways, each app acknowledged only its own command, each second instance exited 0, and Tauri WebView2 children used a smoke-owned user data folder |
...
| WebView visual smoke | Current gallery rerun explicitly launched final SHA-256 `6363339...` through `--exe-path .\release-single\ScreenWatchOCRTauri.exe --gate gallery` and passed template import, target enablement, select-all/invert, row-button reorder, drag/drop reorder, hit-count context menu, delete, clear-all, and capture-source-as-template. Earlier final SHA-256 `426BE3C...` runs passed source preview, clipboard paste, one-shot scan, monitoring restart, layout resize, legacy profile, and late-start remembered-window workflows before the label-only rebuild |
| WebView monitoring soak | previous final SHA-256 `426BE3C...` was explicitly launched through `--exe-path .\release-single\ScreenWatchOCRTauri.exe` and ran profile monitoring for 300,000ms with 150 UI samples, tick delta 562, hit delta 281, progress-log delta 47, and stopped with the button restored to `开始监控` before the target select-all label rebuild |
...
| Persistent monitoring start/stop/status | Proven | monitor session tests, desktop monitoring gates, packaged WebView monitoring restart smoke, packaged 300s monitoring soak | Multi-hour production soak and broad third-party window matrix are not recorded |
...
| Tick/event logs while monitoring | Proven | frontend tests, `monitor-session` contract, packaged WebView restart smoke progress rows, packaged 300s soak with progress-log delta 47 and per-second heartbeat rows while waiting | Log cadence depends on capture speed and configured interval |
