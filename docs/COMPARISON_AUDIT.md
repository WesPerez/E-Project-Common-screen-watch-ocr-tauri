...
Last updated: 2026-07-07 18:06 +08:00
...
- Functional source state: current UI/monitoring fix set plus OCR-lite startup guard
...
| --- | --- |
| Latest continuation audit | Fresh continuation run is recorded in `docs\VERIFICATION_RUN_20260707_1654.md`. It passed full default migration verification, desktop smoke, template benchmark, packaged smoke, Python profile compatibility, template parity, production template smoke, OCR text parity, OCR corpus smoke, final-exe tray smoke, and final-exe Python/Tauri coexistence smoke |
...
| Main migration verifier | Newest focused rerun after app-window self-filter hardening passed Rust formatting, package/script contracts, documentation contracts, command/DOM/event contracts, manual evidence self-test, Rust core 124 / 3 ignored, Tauri 92 / 16 ignored, OCR feature 28 / 2 ignored, and `singleFileDeliverableContract` for the refreshed delivered exe; it records `release-single\ScreenWatchOCRTauri.exe` at 3,587,584 bytes with SHA-256 `200C0C8E...` and WindowsGui subsystem. Earlier current full verifier evidence: Current rerun passed Python 98, Rust core 121 / 3 ignored, Tauri 88 / 16 ignored, OCR feature 25 / 2 ignored, frontend 103, frontend production build, lite release build, dependency boundaries, `requiredRealGates: 19 workspace gates, 2 OCR gates`, and `liteSizeGate: passed`. A follow-up optional rerun passed template benchmark (81ms flat, 457ms textured), packaged smoke, lite portable `target\portable\screen-watch-ocr-tauri-lite-portable-20260707-065219-73bb7825.zip` at 1,616,329 bytes, and full portable `target\portable\screen-watch-ocr-tauri-full-portable-20260707-065500-3652923c.zip` at 3,753,074 bytes. The delivered `release-single\ScreenWatchOCRTauri.exe` is the separately WebView/packaged/tray/coexistence-verified single-file artifact; release builds are not treated as bit-for-bit reproducible evidence. `legacyVisibleWorkflowContract` maps old visible Python workflows to new controls, frontend handlers, and registered Tauri commands; `legacyUiSurfaceContract` locks old visible labels/status/log/layout surfaces to equivalent Tauri UI text, IDs, and layout splitter tests; `legacyDefaultSettingsContract` locks Python GUI defaults against Tauri HTML defaults, frontend build/load fallbacks, and Rust profile defaults; `legacyFailureGuardContract` locks old/new start/scan/capture failure preflights, duplicate-monitoring protection, failed-start UI recovery, and OCR-lite no-worker rejection; `legacyTemplateFileBoundaryContract` locks Python/Rust/Tauri template import, delete, and clear file-boundary behavior so only files under `templates\` are deleted and external target paths are preserved; `legacyProfilePersistenceContract` locks Python `save_state`/`save_current_profile` field placement; `audioAlarmParityContract` locks Python `winsound.PlaySound(..., SND_MEMORY)` behavior against the Tauri runtime; `singleFileDeliverableContract` verifies `release-single\ScreenWatchOCRTauri.exe` size/hash and WindowsGui subsystem |
| Frontend visible UI parity | Focused current rerun passed 103 frontend unit tests after locking the old Python target select-all/invert button behavior. The Tauri target toolbar now starts with `全选`, switches through `profileToggleAllLabel()` to `反选` only when all targets are enabled, `legacyVisibleWorkflowContract` statically checks the old Python `全选`/`反选` source text, new HTML text, frontend helper, and unit test, and `legacyUiSurfaceContract` now verifies the old profile/startup/gallery/source/region/match/run/status/log/preview surface remains represented in the compact resizable Tauri UI |
...
| Packaged smoke | Current rerun against final SHA-256 `200C0C8E...` verified PE subsystem WindowsGui (2), start-minimized, legacy app_data migration, legacy geometry restore, close-to-tray, and second-instance wake with isolated appdata/ports using `release-single\ScreenWatchOCRTauri.exe` |
| Tray menu smoke | Current rerun against final SHA-256 `200C0C8E...` passed Tauri-owned native menu `Show Tauri` and `Exit Tauri`; tray menu PID matched the Tauri PID, exit code was 0, and old Python tray/processes were not touched |
| Python/Tauri coexistence smoke | Current final SHA-256 `200C0C8E...` and old Python SHA-256 `A5689E...` were launched together against one shared isolated `ScreenWatchOCR` data root in `coexistence-smoke-20260707-161624-result.json`. Process trees were distinct (`ScreenWatchOCR` PyInstaller onefile tree vs `ScreenWatchOCRTauri` plus WebView2 tree), both visible main windows were detected, default ports `47627` and `47628` were simultaneously busy, cross-protocol commands were rejected both ways, each app acknowledged only its own command, each second instance exited 0, and Tauri WebView2 children used a smoke-owned user data folder |
...
| WebView visual smoke | Current full rerun `webview-visual-smoke-20260707-161407-result.json` explicitly launched final SHA-256 `200C0C8E...` through `--exe-path .\release-single\ScreenWatchOCRTauri.exe` and passed legacy Python profile restore/scan/monitoring, source preview, template gallery import/edit/reorder/delete/clear/capture with generated PNG/JPG/JPEG/BMP/WebP path imports, clipboard bitmap and file-list paste, one-shot scan with evidence output, OCR-lite raw config rejection, monitoring start/stop/restart with progress logs, and all resizable layout splitter drags. Separate current rerun `webview-visual-smoke-20260707-161653-result.json` also passed the late-start remembered-window workflow after Tauri had already loaded the legacy profile |
| WebView monitoring soak | Current final SHA-256 `200C0C8E...` ran profile monitoring for 600,000ms in `webview-visual-smoke-20260707-161730-result.json` with 300 UI samples, tick delta 2149, hit delta 537, progress-log delta 46, distinct tick counts across the samples, and stopped with the button restored to `开始监控` |
...
| Template benchmark | 2560x1440, 8 templates current verifier rerun: Rust flat 81ms 8/8, Rust textured 457ms 8/8; Python/OpenCV comparison evidence remains flat 46ms 8/8 and textured 45ms with the known 4/8 odd-phase baseline miss |
...
| Template target detection, scales, worker limit | Proven | Rust detector tests, parity/benchmark gates | Current production-profile smoke records about 6.4s for 18 real templates on synthetic 1440p placement; acceptable but worth tracking |
