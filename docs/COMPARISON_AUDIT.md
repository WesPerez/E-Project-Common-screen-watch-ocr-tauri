...
Last updated: 2026-07-07 12:58 +08:00
...
| WebView monitoring soak | Current final SHA-256 `6363339...` ran profile monitoring for 60,000ms with 31 UI samples, tick delta 112, hit delta 112, progress-log delta 47, and stopped with the button restored to `开始监控`. A longer historical 300,000ms run on previous final SHA-256 `426BE3C...` also passed before the target select-all label rebuild |
...
| Persistent monitoring start/stop/status | Proven | monitor session tests, desktop monitoring gates, current packaged WebView monitoring restart smoke, current packaged 60s monitoring soak, historical packaged 300s soak | Multi-hour production soak and broad third-party window matrix are not recorded |
...
| Tick/event logs while monitoring | Proven | frontend tests, `monitor-session` contract, current packaged WebView restart smoke progress rows, current packaged 60s soak with tick delta 112 and progress-log delta 47, plus historical 300s soak | Log cadence depends on capture speed and configured interval |
